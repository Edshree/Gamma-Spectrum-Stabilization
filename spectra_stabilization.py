"""
 - This code is suitable for the stabilization
 - of the scintillator gamma spectrum
 -
 - Author @AiyunSun @MengchengQiu
 - 2021.03.31
 -
"""
...
class Stabilization():
    ...
    def channelIndex(self,peaks,peaks_standard,energys,mode="nonlinear"):
        """
...
if __name__ == "__main__":
    ...
    spec_1 = np.loadtxt("NaI_spectrum_test.txt")
    ...
    sta = Stabilization(spec_1)               # initialization
    peak_H = sta.peaksSearch([210, 240])      # find the H peak in roi [210,240]
    peak_C = sta.peaksSearch([432, 466])      # find the C peak
    peak_Fe = sta.peaksSearch([760, 790])     # find the Fe peak

    print("H peak position: ", peak_H)
    print("C peak position: ", peak_C)
    print("Fe peak position: ", peak_Fe)

    # calculate the channels
    sta.channelIndex([peak_H,peak_C,peak_Fe],[210,419,721],[2.223,4.443,7.638])
    # stabilization
    spec_stabled = sta.stable(2)

    plt.figure()
    plt.plot(spec_1, label="original spectrum")
    plt.plot(spec_stabled, label="stabled spectrum")
    plt.legend()
    plt.show()
